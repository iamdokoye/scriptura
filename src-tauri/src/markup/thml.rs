    ...
    loop {
        match reader.read_event_into(&mut buf) {
            Ok(Event::Empty(ref e)) => {
                ...
                if name == "sync" {
                    ...
                    for attr in e.attributes().flatten() {
                        let k = std::str::from_utf8(attr.key.as_ref()).unwrap_or("").to_lowercase();
                        let v = std::str::from_utf8(attr.value.as_ref()).unwrap_or("").to_string();
                        if k == "type" { sync_type = v.to_lowercase(); }
                        if k == "value" { sync_val = v; }
                    }
            ...
            Ok(Event::Start(ref e)) => {
                let name = std::str::from_utf8(e.name().as_ref()).unwrap_or("").to_lowercase();
                if name == "note" || name == "scripref" { in_note = true; }
            }
            Ok(Event::End(ref e)) => {
                let name = std::str::from_utf8(e.name().as_ref()).unwrap_or("").to_lowercase();
                if name == "note" || name == "scripref" { in_note = false; }
            }
            Ok(Event::Text(ref e)) => {
                if in_note { continue; }
            ...
            Err(_) => {
                ...
                if spans.is_empty() {
                    let stripped = raw.chars()
                        .scan(0i32, |depth, c| {
                            if c == '<' { *depth += 1; Some(None) }
                            else if c == '>' { *depth -= 1; Some(None) }
                            else if *depth == 0 { Some(Some(c)) }
                            else { Some(None) }
                        })
                        .flatten()
                        .collect::<String>();
