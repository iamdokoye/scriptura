...
use crate::markup;
use crate::sword::block::{decompress_zlib, read_u32_le, read_u16_le};
...
impl<'a> BibleReader<'a> {
    pub fn open(module_path: &Path, conf: &ModuleConf, cache: &'a FileCache) -> Result<Self> {
        let data_dir = module_path.join("modules").join("texts")
            .join(conf.data_path.trim_start_matches("./"));
        if !data_dir.exists() {
            let alt = module_path.join(conf.data_path.trim_start_matches("./"));
            if alt.exists() {
                return Ok(Self { conf: conf.clone(), data_dir: alt, cache });
            }
            return Err(AppError::Sword(format!("data dir not found: {}", data_dir.display())));
        }
        Ok(Self { conf: conf.clone(), data_dir, cache })
    }
    ...
    pub fn get_chapter(&self, book: &str, chapter: u32) -> Result<ChapterText> {
        ...
        let (testament, book_ord) = ot_nt_split(book)
            .ok_or_else(|| AppError::Sword(format!("unknown book: {book}")))?;

        let prefix = if testament == 0 { "ot" } else { "nt" };

        let raw_verses = match &self.conf.compression {
            Compression::None => self.rawtext_chapter(prefix, testament, book_ord, chapter, verse_count),
            Compression::Zip  => self.ztext_chapter(prefix, testament, book_ord, chapter, verse_count),
        }?;
    ...
    fn rawtext_chapter(
        &self, prefix: &str, testament: u32, book_ord: u32, chapter: u32, verse_count: u32,
    ) -> Result<Vec<String>> {
        // Each file is loaded from disk once per session; all subsequent calls
        // get an Arc clone pointing to the same allocation.
        let vss = self.cache.read(&self.data_dir.join(format!("{prefix}.vss")))?;
        let bdt = self.cache.read(&self.data_dir.join(format!("{prefix}.bdt")))?;
        ...
        for verse in 1..=verse_count {
            let ordinal = match kjv_ord(self, testament, book_ord, chapter, verse) {
                Some(v) => v, None => { out.push(String::new()); continue; }
            };
            let entry = ordinal * 6;
            let data_off = match read_u32_le(&vss, entry) { Some(v) => v as usize, None => { out.push(String::new()); continue; } };
            let size     = match read_u16_le(&vss, entry + 4) { Some(v) => v as usize, None => { out.push(String::new()); continue; } };
            if size == 0 { out.push(String::new()); continue; }
    ...
    fn ztext_chapter(
        ...
        let bzv = self.cache.read(&self.data_dir.join(format!("{prefix}.bzv")))?;
        let bzs = self.cache.read(&self.data_dir.join(format!("{prefix}.bzs")))?;
        let bdt = self.cache.read(&self.data_dir.join(format!("{prefix}.{ext}")))?;
        ...
        for verse in 1..=verse_count {
            ...
            let bze = ordinal * 10;
            let block_num   = match read_u32_le(&bzv, bze)     { Some(v) => v as usize, None => { out.push(String::new()); continue; } };
            let verse_start = match read_u32_le(&bzv, bze + 4) { Some(v) => v as usize, None => { out.push(String::new()); continue; } };
            let verse_size  = match read_u16_le(&bzv, bze + 8) { Some(v) => v as usize, None => { out.push(String::new()); continue; } };
            if verse_size == 0 { out.push(String::new()); continue; }

            let decompressed = if let Some(d) = block_cache.get(&block_num) {
                d
            } else {
                let bse = block_num * 12;
                let boff = match read_u32_le(&bzs, bse)     { Some(v) => v as usize, None => { out.push(String::new()); continue; } };
                let blen = match read_u32_le(&bzs, bse + 4) { Some(v) => v as usize, None => { out.push(String::new()); continue; } };
                let compressed = match bdt.get(boff..boff + blen) { Some(b) => b, None => { out.push(String::new()); continue; } };
                match decompress_zlib(compressed) {
                    Ok(d)  => { block_cache.insert(block_num, d); block_cache.get(&block_num).unwrap() }
                    Err(_) => { out.push(String::new()); continue; }
                }
            };

            out.push(match decompressed.get(verse_start..verse_start + verse_size) {
                Some(b) => self.decode(b).unwrap_or_default(),
                None    => String::new(),
            });
        }
    ...
    fn rawtext_verse(&self, prefix: &str, testament: u32, book_ord: u32, chapter: u32, verse: u32) -> Result<String> {
        let vss = self.cache.read(&self.data_dir.join(format!("{prefix}.vss")))?;
        let bdt = self.cache.read(&self.data_dir.join(format!("{prefix}.bdt")))?;
        let ordinal = self.kjv_ordinal(testament, book_ord, chapter, verse)? as usize;
        let entry = ordinal * 6;
        let off  = read_u32_le(&vss, entry)     .ok_or_else(|| AppError::Sword("vss out of range".into()))? as usize;
        let size = read_u16_le(&vss, entry + 4) .ok_or_else(|| AppError::Sword("vss out of range".into()))? as usize;
        if size == 0 { return Ok(String::new()); }
        self.decode(bdt.get(off..off + size).ok_or_else(|| AppError::Sword("bdt out of range".into()))?)
    }

    fn ztext_verse(&self, prefix: &str, testament: u32, book_ord: u32, chapter: u32, verse: u32) -> Result<String> {
        let ext = match &self.conf.block_type { BlockType::Book => "bzz", BlockType::Chapter => "bzc", BlockType::Verse => "bzt" };
        let bzv = self.cache.read(&self.data_dir.join(format!("{prefix}.bzv")))?;
        let bzs = self.cache.read(&self.data_dir.join(format!("{prefix}.bzs")))?;
        let bdt = self.cache.read(&self.data_dir.join(format!("{prefix}.{ext}")))?;
        let ordinal = self.kjv_ordinal(testament, book_ord, chapter, verse)? as usize;
        let bze = ordinal * 10;
        let block_num   = read_u32_le(&bzv, bze)     .ok_or_else(|| AppError::Sword("bzv: block_num".into()))? as usize;
        let verse_start = read_u32_le(&bzv, bze + 4) .ok_or_else(|| AppError::Sword("bzv: verse_start".into()))? as usize;
        let verse_size  = read_u16_le(&bzv, bze + 8) .ok_or_else(|| AppError::Sword("bzv: verse_size".into()))? as usize;
        if verse_size == 0 { return Ok(String::new()); }
        let bse         = block_num * 12;
        let boff        = read_u32_le(&bzs, bse)     .ok_or_else(|| AppError::Sword("bzs: offset".into()))? as usize;
        let blen        = read_u32_le(&bzs, bse + 4) .ok_or_else(|| AppError::Sword("bzs: size".into()))? as usize;
        let compressed  = bdt.get(boff..boff + blen) .ok_or_else(|| AppError::Sword("bdt: slice".into()))?;
        let decompressed = decompress_zlib(compressed)?;
        self.decode(decompressed.get(verse_start..verse_start + verse_size).ok_or_else(|| AppError::Sword("decomp: slice".into()))?)
    }
    ...
    fn kjv_ordinal(&self, testament: u32, book_ord: u32, chapter: u32, verse: u32) -> Result<u32> {
        ...
        kjv_verse_ordinal(testament, book_ord, chapter, verse)
            .ok_or_else(|| AppError::Sword(format!("out-of-range: t={testament} b={book_ord} ch={chapter} v={verse}")))
    }
}

/// Helper to avoid a `self.` borrow conflict inside iterator closures.
#[inline]
fn kjv_ord(_reader: &BibleReader<'_>, testament: u32, book_ord: u32, chapter: u32, verse: u32) -> Option<usize> {
    use crate::versification::kjv_verse_ordinal;
