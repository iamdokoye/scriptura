...
static REF_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\b([1-3]?\s*[A-Za-z]{2,8})\s+(\d+):(\d+)(?:-(\d+))?").unwrap()
});
...
fn strip_markup(text: &str) -> String {
    ...
    while let Some(ch) = chars.next() {
        match ch {
            '<' => { while chars.next_if(|&c| c != '>').is_some() {} chars.next(); }
            '\\' => { while chars.next_if(|&c| c != '\\' && c != ' ').is_some() {} chars.next_if(|&c| c == '\\'); }
            _ => out.push(ch),
        }
    }
    out
...
pub fn parse_tsk_text(text: &str, max: usize) -> Vec<CrossReference> {
    ...
    for cap in REF_RE.captures_iter(&clean) {
        ...
        let chapter: u32 = match cap[2].parse() { Ok(n) => n, Err(_) => continue };
        let verse: u32  = match cap[3].parse() { Ok(n) => n, Err(_) => continue };
        let end_verse: Option<u32> = cap.get(4).and_then(|m| m.as_str().parse().ok());

        if chapter == 0 || verse == 0 { continue; }

        let Some(book) = resolve_book(book_raw) else { continue };
